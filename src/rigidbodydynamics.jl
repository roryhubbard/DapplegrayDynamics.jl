function doublependulum()::Mechanism{T}
    ...
    axis = SVector(T, 0., 1., 0.) # joint axis
    ...
    inertia2 = SpatialInertia(CartesianFrame3D("lower_link"),
        ...
        com=SVector(T, 0, 0, c_2),
        mass=m_2)
    lowerlink = RigidBody(inertia2)
    elbow = Joint("elbow", Revolute(axis))
    before_elbow_to_after_shoulder = Transform3D(
        frame_before(elbow), frame_after(shoulder), SVector(T, 0, 0, l_1))
...
function simulate_mechanism(mechansim::Mechanism{T}, finaltime::T, Δt::T, initial_configuration::AbstractVector{T}, initial_velocity::AbstractVector{T}) where {T}
